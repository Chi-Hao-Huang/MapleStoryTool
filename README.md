...
7. **斷線重連模組 (Reconnect Module)**
   ...
   - **定時重啟模組**：腳本執行過久，遊戲用戶端可能累積一些不容易單獨排查的異常（記憶體洩漏、連線不穩等），所以每隔 `cfg.restart_interval_minutes`（預設 60 分鐘）就會主動觸發一次完整重連流程，跟斷線觸發走的是同一套 `ReconnectAttemptTracker` 邏輯。計時從腳本啟動開始算，每次重連完成（不論是斷線觸發、定時觸發、還是下面的找不到視窗觸發）都會重新歸零；設為 0 或負數可停用。
   - **連續找不到遊戲視窗也會觸發重啟**：主迴圈每個 tick 一開始都會用 `get_game_window` 更新視窗座標，找不到時（例如遊戲當掉、視窗被關閉）會累加計數，連續達到 `max_consecutive_window_not_found`（預設 10 次）就直接觸發跟斷線一樣的重連流程；只要視窗重新找得到，計數就會歸零。
   - **需要人工排除的狀況改發提示音，不自動重啟**：遊戲不定時彈出的 **LIE DETECTOR 防外掛檢測畫面**（`is_lie_detector_open`，範本圖 `lie_detector_template`，預設 `image/lie_detector_notice.png`）需要玩家動滑鼠配合才能通過，程式無法也不應該自動處理；連續 `max_consecutive_player_not_found` 次偵測不到玩家標籤，原因也不明（可能卡在某個對話框、視窗被切走、角色標籤模板失效）。這兩種情況都改成 `keyup_all()` 停止動作後呼叫 `play_alert_sound` 發出提示音（`winsound.Beep`，頻率/長度對應 `alert_beep_frequency` / `alert_beep_duration_ms`），而不是強制重啟遊戲——重啟也一樣通不過 LIE DETECTOR 檢測，另一種狀況原因不明時貿然重啟也可能沒有幫助。`AlertThrottler` 會依 `alert_repeat_interval_seconds` 持續提醒，直到狀況解除（LIE DETECTOR 畫面消失、或重新偵測到玩家）才自動停止。
...
10. **跨平台爬繩模組 (Rope Traverser)**
    ...
    - 用 `LayerConfig` 定義地圖有哪些平台：Y 座標範圍 + 該層專屬的左右巡邏邊界；用 `RopeConfig` 定義平台之間的連接：上/下層 index、連接類型 `connection_type`（`"rope"` 實體繩索或 `"flash_jump"` 瞬間移動技能，見下方說明）、以及 `"rope"` 類型才需要的 X 座標。兩者集中設定在 `BotConfig.layers` / `BotConfig.ropes`。
    ...
    - 主迴圈依目前所在平台計算該層的巡邏邊界，藉此讓角色依序走遍地圖上所有已設定的平台並持續打怪；`min_seconds_between_climbs` 避免剛完成動作又立刻折返、`post_transition_cooldown` 讓爬繩/掉落動畫播完再恢復巡邏判斷、`climb_timeout_seconds` 則是卡住時的逾時保護。
    - **往上爬 vs 往下掉落的觸發條件不同**：往下掉落（`down` + 跳躍鍵）是瞬間動作，平台上任何位置都能觸發，不需要對齊，所以用 `find_rope_down_from_layer` 只看「這一層有沒有往下的連接」、不檢查 X 座標——有些繩索的 X 座標其實落在下層平台自己的巡邏邊界之外，這種情況單靠 X 對齊永遠不會觸發。往上爬則依 `connection_type` 而不同：`"rope"` 需要先主動走到繩索的確切 X 座標才能抓到繩子（見下方 `pick_up_rope`）；`"flash_jump"` 則跟往下掉落一樣是瞬間動作，只要落在**上層平台自己的** `left_bound`/`right_bound` 範圍內（不需要對齊到某個確切點）就能按 `climb_up_key` + `flash_jump_key` 觸發，適合角色本身有瞬間移動/跳躍技能、平台間距不需要靠繩索的地圖；不在範圍內時一樣會主動朝該平台的方向走過去，而不是被動等一般巡邏折返剛好經過。`RopeTraverser` 內部會依連接類型自動決定要不要走 `align`/`grab` 這些多 tick 的爬繩流程，還是像掉落一樣觸發後立刻視為完成。
    - **`LayerSweepDirector`（決定往上還是往下、以及分岔時走哪一條）**：因為往下掉落不需要對齊、永遠比往上爬更容易觸發，如果每次都優先判斷「能不能往下掉」，角色會一直卡在下面幾層來回移動、永遠上不到最上層。`LayerSweepDirector.decide` 改用類似磁碟排程 SCAN 演算法的邏輯：記住目前是在「往上」還是「往下」掃描，同一個方向能繼續走就繼續走，直到那個方向已經沒有平台可以再移動（到頂或到底）才反過來，讓角色像電梯一樣完整掃過所有平台。
      - **支援分岔地圖**：地圖不一定是單純一條線排列——一個平台可以同時往上連到多個平台（例如地面連接左右兩側各自往上的分岔路線，`RopeConfig` 可以有多筆 `lower_layer` 相同、`upper_layer`/`x` 不同的紀錄）。`decide` 只判斷「這個方向還有沒有平台可以走」，不管幾條分岔都算數，SCAN 邏輯本身不受影響；真正該挑哪一條分岔則由 `LayerSweepDirector.pick_up_rope` 用輪流（round-robin）方式決定，確保每個分岔都有機會被走到，不會因為某條繩索剛好離平台巡邏路徑比較近就永遠只走那一條。選定目標後，角色會主動朝繩索的 X 座標移動（而不是被動等一般巡邏折返剛好經過），對齊後才真正觸發爬繩；直到成功開始爬繩才會清掉這次選擇（`clear_pending`），讓下次到同一個分岔點時輪到下一條。
      - 單純一條線排列的地圖（例如 3 層：0→1→2）是分岔數為 1 的特例，行為跟原本完全一樣，會呈現 0→1→2→1→0→1→2→1→0→... 的巡迴，依序走遍每一層。
    ...
    - **其他玩家 / 隊友避讓**：確定要爬繩換層前，會用 `get_other_player_minimap_positions` / `get_teammate_minimap_positions` 比對小地圖上「其他玩家」(`#EE0000` 紅色) 與「隊友」(`#FF7700` 橘色) 的色點，透過 `find_occupied_layers` 換算成所在平台。若這次要移動過去的目標平台已經有其他玩家或隊友，就放棄這次換層、留在原地繼續巡邏，下次滿足巡邏次數門檻時會再重新判斷一次；此行為由 `cfg.detect_other_players` 開關控制。這兩種色點的 HSV 顏色範圍是遊戲固定顯示色，比照小地圖自己黃點 (`find_player_on_minimap`) 的作法直接寫死在對應函式內，不放進 `BotConfig`。
      - **形狀過濾**：`find_colored_dots_on_minimap` 除了顏色與面積，還會用「4×π×面積/周長²」算圓形度（正圓為 1），濾掉箭頭、星芒狀圖示等顏色雖然符合但形狀不是圓點的畫面元素，門檻是 `min_circularity`（預設 0.6）。
...
└── image/
    ...
    ├── lie_detector_notice.png      # LIE DETECTOR 防外掛檢測畫面特徵
