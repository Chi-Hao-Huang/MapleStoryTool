...
7. **斷線重連模組 (Reconnect Module)**
   ...
   - 同樣用模板比對偵測遊戲不定時彈出的 **LIE DETECTOR 防外掛檢測畫面**(`is_lie_detector_open`，範本圖 `lie_detector_template`，預設 `image/lie_detector_notice.png`)。這個檢測需要玩家動滑鼠配合，掛機時沒有人操作一定無法通過，所以策略是偵測到就直接觸發跟斷線一樣的「強制關閉遊戲 → 重新連線」流程，而不是嘗試自動配合檢測。
   - 偵測到斷線（或 LIE DETECTOR 檢測畫面）後自動：強制關閉舊的遊戲進程 (`force_close_game`) → 開啟/喚醒 Chrome 並依比例座標點擊登入官網、gamapass 登入、選擇角色 → 等待伺服器選擇畫面出現 → 選伺服器、選頻道、進入遊戲。
   ...
   - **連續偵測不到玩家位置也會觸發重啟**：主迴圈每次偵測不到玩家標籤都會累加計數，連續達到 `max_consecutive_player_not_found`（預設 20 次）就視為畫面異常（例如卡在某個對話框、視窗被切走、角色標籤模板失效），直接觸發跟斷線一樣的強制重啟流程；只要成功偵測到玩家一次，計數就會歸零重算。
...
10. **跨平台爬繩模組 (Rope Traverser)**
    ...
    - **往上爬 vs 往下掉落的觸發條件不同**：往上爬需要對齊繩索的確切 X 座標（`find_rope_near_x` + `rope_x_tolerance`）才能抓到繩子；往下掉落（`down` + 跳躍鍵）是瞬間動作，平台上任何位置都能觸發，不需要對齊，所以用 `find_rope_down_from_layer` 只看「這一層有沒有往下的連接」、不檢查 X 座標——有些繩索的 X 座標其實落在下層平台自己的巡邏邊界之外，這種情況單靠 X 對齊永遠不會觸發。
    - **`LayerSweepDirector`（決定往上還是往下）**：因為往下掉落不需要對齊、永遠比往上爬更容易觸發，如果每次都優先判斷「能不能往下掉」，角色會一直卡在下面幾層來回移動、永遠上不到最上層。`LayerSweepDirector` 改用類似磁碟排程 SCAN 演算法的邏輯：記住目前是在「往上」還是「往下」掃描，同一個方向能繼續走就繼續走，直到那個方向已經沒有平台可以再移動（到頂或到底）才反過來，讓角色像電梯一樣完整掃過所有平台（例如 3 層地圖會呈現 0→1→2→1→0→1→2→1→0→... 的巡迴，等於依序走遍每一層）。這個機制假設平台像樓層一樣「一條線」排列、沒有分岔，符合目前地圖的接法。
