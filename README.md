...
10. **跨平台爬繩模組 (Rope Traverser)**
    ...
    - **往上爬 vs 往下掉落的觸發條件不同**：往上爬需要對齊繩索的確切 X 座標（`find_rope_near_x` + `rope_x_tolerance`）才能抓到繩子；往下掉落（`down` + 跳躍鍵）是瞬間動作，平台上任何位置都能觸發，不需要對齊，所以用 `find_rope_down_from_layer` 只看「這一層有沒有往下的連接」、不檢查 X 座標——有些繩索的 X 座標其實落在下層平台自己的巡邏邊界之外，這種情況單靠 X 對齊永遠不會觸發。每個 tick 會優先判斷「能不能立刻往下掉」，只有掉落選項正在冷卻中（`RopeTraverser.can_start`，見下方 `min_seconds_between_climbs`）或不存在時，才改判斷往上爬；這個優先序搭配 `can_start` 的冷卻機制，讓角色剛爬上來後有機會先往其他方向探索，而不是立刻又掉回去。
