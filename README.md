...
10. **跨平台爬繩模組 (Rope Traverser)**
    ...
    - **往上爬 vs 往下掉落的觸發條件不同**：往下掉落（`down` + 跳躍鍵）是瞬間動作，平台上任何位置都能觸發，不需要對齊，所以用 `find_rope_down_from_layer` 只看「這一層有沒有往下的連接」、不檢查 X 座標——有些繩索的 X 座標其實落在下層平台自己的巡邏邊界之外，這種情況單靠 X 對齊永遠不會觸發。往上爬則需要先主動走到繩索的確切 X 座標才能抓到繩子（見下方 `pick_up_rope`）。
